...
class GreyWolfOptimizer(Algorithm):
	...
	def setParameters(self, NP=25,A_val = 2,C_val = 2, **ukwargs):
		...
		Returns:
			Tuple[numpy.ndarray, numpy.ndarray, numpy.ndarray, float, Dict[str, Any]]:
				...
				3. Additional arguments:
					* A (): TODO
		"""
		a = self.A_val - task.Evals * (self.A_val / task.nFES)
		for i, w in enumerate(pop):
			A1, C1 = 2 * a * self.rand(task.D) - a, self.C_val * self.rand(task.D)
			X1 = A - A1 * fabs(C1 * A - w)
			A2, C2 = 2 * a * self.rand(task.D) - a, self.C_val * self.rand(task.D)
			X2 = B - A2 * fabs(C2 * B - w)
			A3, C3 = 2 * a * self.rand(task.D) - a, self.C_val * self.rand(task.D)
			X3 = D - A3 * fabs(C3 * D - w)
